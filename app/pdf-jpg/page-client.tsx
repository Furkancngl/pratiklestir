...
const accentClassName =
  tools.find((tool) => tool.href === "/pdf-jpg")?.accentClassName ?? "bg-fuchsia-500";
...
export default function PdfJpgPage() {
  ...
  const { checkAndConsume, creditError } = useCreditGate("/pdf-jpg");
  const { maxFileSizeBytes, buildOversizedError } = useFileSizeLimit();

  const baseName = file ? file.name.replace(/\.pdf$/i, "") : "sayfa";
  ...
  const handleConvert = async () => {
    ...
    try {
      const { loadPdfDocument, renderPageToBlob } = await import("../lib/pdf-render");
      const pdf = await loadPdfDocument(bytes.slice());

      const newResults: PageResult[] = [];
      for (let page = 1; page <= pageCount; page++) {
        const blob = await renderPageToBlob(pdf, page);
        newResults.push({ page, blob, url: URL.createObjectURL(blob) });
      }
      setResults(newResults);
    } catch {
      setError("PDF görsellere dönüştürülürken bir sorun oluştu. Lütfen tekrar deneyin.");
    } finally {
      setIsProcessing(false);
    }
  };

  const downloadResult = (result: PageResult) => {
    const link = document.createElement("a");
    link.download = `${baseName}-sayfa-${result.page}.jpg`;
  ...
  const downloadAllAsZip = async () => {
    ...
    results.forEach((result) => {
      zip.file(`${baseName}-sayfa-${result.page}.jpg`, result.blob);
    });
    const zipBlob = await zip.generateAsync({ type: "blob" });
    const url = URL.createObjectURL(zipBlob);
    const link = document.createElement("a");
    link.download = `${baseName}-jpg.zip`;
    link.href = url;
    link.click();
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="mt-6 flex flex-col items-center gap-2 text-center">
          ...
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            PDF dosyanızı yükleyin, her sayfa ayrı bir JPG görsel olarak dışa
            aktarılsın.
          </p>
        </div>
          ...
          <>
            <ul className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
              {results.map((result) => (
                <li
                  key={result.page}
                  className="flex flex-col gap-2 rounded-xl border border-black/[.08] bg-white p-2 dark:border-zinc-800 dark:bg-zinc-950"
                >
                  <div className="flex h-28 items-center justify-center overflow-hidden rounded-lg bg-zinc-100 dark:bg-zinc-900">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={result.url}
                      alt={`Sayfa ${result.page}`}
                  ...
                  <button
                    ...
                    className="rounded-lg border border-black/[.08] px-2 py-1.5 text-xs font-medium text-zinc-700 transition-colors hover:bg-black/[.04] dark:border-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-800"
                  >
                    Sayfa {result.page} · İndir
                  </button>
                </li>
              ))}
            </ul>
